...
这份蓝图描述如何把早期 `design-spec/` 形态演进为一套面向 DDD 业务规格的泛用工具。
...
`ddd-spec doctor` 可以作为后续增强项，但不是当前泛用化里程碑的必需交付物。
...
- 明确 `schemaVersion` 或等价版本字段
- 明确 breaking change 必须升版本
- 明确 v1 暂不承诺自动 migration
...
- 支持从外置 spec source 加载任意符合 contract 的 viewer JSON
- 保留当前 repo-local generated JSON 作为默认输入，而不是唯一输入
- 支持 view 切换
- 明确 loading / error 状态
- 只消费 viewer contract，不内嵌 canonical 解析逻辑

下面这些能力可以明确后置为产品增强项，而不是“通用加载器”成立的前置条件：
...
当前里程碑至少维护一个共享 fixture 和一个 example domain：

- `test/fixtures/connection-card-review`
- `examples/order-payment`

`content-moderation` 可以作为下一阶段压测用的第三个 example，而不是当前里程碑的阻塞项。

如果接入第二个 example 时就持续需要修改 core 语义，说明抽象尚未稳定；第三个 example 的价值主要是进一步降低“单一样本偏差”。
...
- 删除 core 中的领域特定导出便利层
- 把 `connection/card` 相关 helper 下沉到 fixture 或 example
- 明确 core、projection、fixture、example 的目录边界
...
- 引入第二个 example domain
- 给 bundled spec JSON 和 analysis JSON 补 golden tests
- 给 schema validation 和 semantic validation 补独立测试
- 固化 schema versioning 的最小策略
- 固化 diagnostic contract 的最小策略
- 升级 viewer 为最小通用加载器

可以明确后置的动作：

- 引入第三个 example domain
- 编写完整 migration guide
- 增加 `ddd-spec doctor`
- 给 viewer 增加远程 URL / 本地上传 / 多 spec / deep link / 过滤 / 高亮链路等产品功能

交付物：

- `examples/order-payment`
- bundled spec / analysis / schema / semantic tests
- 带最小版本边界说明的 schema / diagnostic / viewer contract
- 最小通用 viewer app

Phase 4 完成后，如果还要继续压测泛用性，再进入 Phase 4b：
...
3. 补第二个 example domain，用它验证抽象边界。
4. 把 viewer 改成可切换外置 spec source 的最小通用加载器。
...
- 先尽快引入第二个 example
- 第三个 example 作为进一步压测，而不是当前里程碑的前置条件
...
- 至少两个 example domain 可以共用同一套 core 和 projections
- viewer 可以从可配置 spec source 加载任意生成的 viewer spec，而不是绑定本仓库固定路径
- schema、diagnostic code、viewer contract 都有最小版本边界和回归测试

如果能再接入第三个 example domain，则说明这套抽象的稳定性更强，但不是当前里程碑的必要条件。
