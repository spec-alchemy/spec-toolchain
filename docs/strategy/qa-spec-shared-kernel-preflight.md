...
- 当前 skeleton 足以支撑 `qa-spec` 的第一轮边界讨论，但还不足以支撑其成为正式 `spec family` 的前置条件；缺口主要在 cross-family `stable ID` 引用、`traceability` `contract`、evidence attachment 和 gate result envelope。
...
| --- | --- | --- |
| cross-family `stable ID` reference `contract` | 现在只有 diagnostics related resource shape，还没有正式 shared 引用语义 | 是否需要独立 shared reference type，覆盖 family、kind、stable ID、version hint 和失效诊断 |
...
2. 还没有 cross-family `stable ID` 引用语义，`qa-spec` 无法可靠链接到 `ui-spec`、`frontend-spec`、`ddd-spec` 的 canonical 对象。
3. `execution` 仍然故意留空，这是正确的范围控制，但意味着 QA 目前还不能证明第二个 runtime surface 的可落地接缝。
