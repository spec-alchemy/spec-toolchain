...
- 如果旧的 `vNext` / `canonical` 文档与本文件冲突，以本文件为准；旧文档只保留产品背景或待清理历史语义。
...
- `canonical-vnext/`、`ddd-spec/canonical-vnext/`、`vnext-*` 都不再属于默认用户路径。
...
- repo-private maintainer 默认示例与 viewer 演示路径，不得继续使用 `canonical-vnext` 或 `vnext-*` 作为默认命名。
