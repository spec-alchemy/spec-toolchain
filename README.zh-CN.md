...
| 任务 | 命令 |
|------|------|
| 验证仓库示例流程 | `npm run check:repo:validate` |
| 构建仓库示例输出 | `npm run check:repo:build` |
| 启动打包 viewer 示例流程 | `npm run ops:repo:viewer` |
| 运行包级回归测试 | `npm run check:pkg:test` |
| 构建 viewer gate surface | `npm run check:app:build` |
| 运行基础 PR gate | `npm run gate:pr` |
| 运行发布就绪 gate | `npm run gate:release` |
| 模拟发布操作路径 | `npm run ops:release:dry-run` |
